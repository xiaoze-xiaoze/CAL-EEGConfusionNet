...
class EEGConfusionNet(nn.Module):
    def __init__(self, n_channels=8, seq_len=250, n_classes=3, conv_filters=64, lstm_hidden=128, dropout=0.3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = torch.cat([c3, c5, c7], dim=1)    # (batch, filters*3, time)
        x = self.bn(x)

        # GLU门控
        x = x.transpose(1, 2)                 # (batch, time, filters*3)
        x = self.glu(x)
        x = self.dropout1(x)
        x = x.transpose(1, 2)                 # (batch, filters*3, time)
        ...
        # 时间注意力
        x = self.temporal_attn(x)             # (batch, hidden*2)
